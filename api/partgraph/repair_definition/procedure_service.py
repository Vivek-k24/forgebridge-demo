...
from .procedure_schemas import ProcedureActionRead, RepairProcedureRead
    ...
    while len(resolved) < len(actions):
        eligible = sorted(
            (
                ...
                and remaining[action.id].issubset(resolved)
            ),
...
async def _procedure_for_definition(
    ...
    *,
    ...
    evidence_rows = (
        await session.execute(
            ...
            .where(
                ...
                MechanicalClaim.vehicle_configuration_id
    ...
    missing_evidence = [
        action.id for action in actions if not claims_by_action[action.id]
    ]
    if missing_evidence:
        raise _integrity_error(
