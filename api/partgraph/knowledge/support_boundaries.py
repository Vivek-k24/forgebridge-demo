...
def is_computer_service_boundary(action_key: str) -> bool:
    """Return True only for the reserved terminal computer-service boundary."""
...
def validate_computer_service_boundary(
    ...
    """Enforce the invariant for partially guided repairs.

    A computer-dependent operation is not a guided PartGraph action.  The
    canonical procedure may contain one reserved terminal boundary so the user
    can complete all verified physical work and then be told clearly that the
    remaining required operation is unsupported.  The boundary must be last
    and cannot be skippable.
    """
