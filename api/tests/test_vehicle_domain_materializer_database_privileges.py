...
class VehicleDomainMaterializerPrivilegeTests(unittest.TestCase):
    @classmethod
    ...
    def test_materializer_can_append_vehicle_domains_but_cannot_mutate_them(self) -> None:
        for table in (
            "public.vehicle_structure_nodes",
            "public.specification_definitions",
            "public.vehicle_specification_values",
        ):
            with self.subTest(table=table):
                ...
                self.assertFalse(self._table_privilege(MATERIALIZER_ROLE, table, "UPDATE"))
