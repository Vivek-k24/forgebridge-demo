...
def test_workbook_batch_reports_exact_production_collection_scope() -> None:
    ...
    assert body["collected"] == 363
    assert body["verified"] == 0
    assert body["conflict"] == 0
    assert body["remaining"] == 363
    ...
    assert {row["make"]: row["collected"] for row in body["makes"]} == EXPECTED_MAKES
    assert all(row["verified"] == 0 for row in body["makes"])
    assert all(row["remaining"] == row["collected"] for row in body["makes"])
...
def test_catalog_coverage_batch_list_contains_workbook_batch() -> None:
    ...
    assert workbook["collected"] == 363
    assert sum(row["collected"] for row in workbook["makes"]) == 363
