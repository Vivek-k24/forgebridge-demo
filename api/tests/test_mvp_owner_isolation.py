...
class FinalMvpOwnerIsolationTests(unittest.TestCase):
    ...
    def setUpClass(cls) -> None:
        ...
        cls.admin_connection = psycopg.connect(cls.database_url, autocommit=True)
    ...
    def setUp(self) -> None:
        self.app_connection = psycopg.connect(self.database_url)
        self.owner_a = uuid4()
        self.owner_b = uuid4()
        self.owner_c = uuid4()
    ...
    def _visible_preference_owners(self, owner_id=None) -> set[str]:
        with self.app_connection.transaction():
            with self.app_connection.cursor() as cursor:
