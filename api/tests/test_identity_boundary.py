...
def test_legacy_identity_paths_resolve_to_identity_owned_orm_classes() -> None:
    ...
    assert (
        legacy_vehicle_models.VehicleConfiguration
        is identity_vehicle_models.VehicleConfiguration
    )
    assert legacy_user_vehicle_models.UserVehicle is identity_user_vehicle_models.UserVehicle
    assert (
        legacy_user_vehicle_models.VinDecodeCache
        is identity_user_vehicle_models.VinDecodeCache
    )

    assert identity_auth_models.User.__module__ == "partgraph.identity.auth.models"
    assert (
        identity_vehicle_models.VehicleConfiguration.__module__
        == "partgraph.identity.vehicle.models"
    )
    assert (
        identity_user_vehicle_models.UserVehicle.__module__
        == "partgraph.identity.user_vehicle.models"
    )
...
def test_legacy_identity_paths_do_not_duplicate_sqlalchemy_tables() -> None:
    ...
    assert (
        legacy_user_vehicle_models.UserVehicle.__table__
        is identity_user_vehicle_models.UserVehicle.__table__
    )
    assert (
