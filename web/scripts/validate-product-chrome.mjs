...
requireText(auth, "type={passwordVisible ? 'text' : 'password'}", 'show/hide password input type')
requireText(auth, "aria-label={passwordVisible ? 'Hide password' : 'Show password'}", 'password visibility accessible name')
requireText(auth, 'aria-pressed={passwordVisible}', 'password visibility pressed state')
requireText(auth, "setPasswordVisible(false)", 'password visibility reset-to-hidden behavior')
